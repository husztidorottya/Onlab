...
def show_mispredictions(test_words, test_cat, X_test, Y_test, model):
    ...
    with open('bad_predictions.tsv', 'w') as file:
        for i in range(0,len(predictions)):
            if predictions[i] != Y_test[i]:
                # word - expected - prediction
                expected = get_derivations(Y_test[i])
                prediction = get_derivations(predictions[i])
                file.write('{0}\t{1}\t{2}\n'.format(test_words[i],expected,prediction))

"""
Get the derivations according to predictions
"""
def get_derivations(value):
    if value == 0:
        expected = '[/N/N]'
    if value == 1:
        expected = '[/N/Adj]'
...
def main():
    ...
    derivations_format3 = input('Derivaciok:')
    ...
    with open('log_experiment.tsv','w') as logfile:
         ...
         logfile.write('Derivations3{0}\n'.format(derivations_format3))
    ...
    for round in range(0,int(args.experiment_number)):
        ...
        for i in range(0,3):
            ...
            if i == 1:
                ...
                derivations = derivations_format3.replace('\t','[^/]*').replace('\t','[^/]*').replace('\t','[^/]*').replace(';','[^a-zA-Z]')
            rules = '^' + derivations + '[^/]*$'
